...
class LiveDataTestObserver<T> : Observer<T> {
    ...
    fun assertValueSequence(
        ...
        if (hasNextActual) {
            throw AssertionError("More values received than expected(actual: ${recordedActualValues}, expected: ${expectedValues}).")
        }
        if (hasNextExpected) {
            throw AssertionError("Fewer values received than expected(actual: ${recordedActualValues}, expected: ${expectedValues}).")
        }
